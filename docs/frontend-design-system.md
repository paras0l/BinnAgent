...
- `KnowledgeBasePage` 必须保持学习者视角：普通学习端只展示今日单元、练习任务、教材信息和学习路径；解析校对、解析质量、教材结构内部产物、parser/ingest 证据、低置信词条确认入口和调试表格统一放入 Dev Console 的 Textbook Parsing。
...
- 学习设置中的练习偏好使用 learner-scoped localStorage；学习目标和当前水平写入 LearnerProfile，prompt 读取画像后按目标和 CEFR 水平调整难度。词汇练习默认模式、数量、发音、进入前设置页、自动播放、拼写自动检查、答对后自动进入下一题、单元词汇范围都应从本地练习偏好读取。

## PageShell

核心学习页面统一使用浅灰工作区背景和居中内容宽度：

- 外层背景：使用 `binn-page-canvas` 的轻冷色环境层；光晕只用于营造空间，不承载信息。
- 页面宽度：`max-w-[1180px]`
...
卡片只用于“可独立操作、可单独理解”的内容单元。连续步骤、筛选器、说明文字和同组统计优先使用分隔线、浅色分区或列表，不再逐项套卡片。页面先建立留白与阅读节奏，再决定是否需要边框。
