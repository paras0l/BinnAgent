...
| ParserRun / QualityGate / ReviewQueue | 教材 ingest 已改为后台任务 + 状态轮询；新增 DocumentParseArtifact、ParserRouter、MarkItDown baseline、pypdf fallback、文档质量指标、TextbookExtractor artifact 候选抽取；Dev Console 可看 parsing report/evidence/review queue；golden parser evaluation 已有 MVP | layout-aware extractor、完整 OCR、多年级 golden profile、持久化任务队列未完成 | 保留为知识冷启动与质量治理，不继续扩展教材硬编码路线 | 教材解析是知识来源治理，不是产品主卖点；重点是防止低质量知识静默进入学习闭环 |
...
| Knowledge Base / RAG | 部分实现 | 文档解析流水线第一阶段已落地：MarkItDown baseline、pypdf fallback、统一 DocumentParseArtifact、DocumentQuality、ParserRouter attempted/selected/fallback 记录、后台 ingest + ingest-status 轮询、metadata 状态拆分；教材抽取层改为消费 artifact 候选；另有 chunk、embedding、文本 fallback、ParserRun 审计、ParserReviewItem 队列、Parser Evidence 目标查询、Golden Dataset + Parser Evaluation MVP、8 题混合练习流、hint/retry/rubric 反馈 | OCR pipeline、layout-aware extractor、hybrid retrieval、golden query set 扩展、练习 session 总结、更多年级 golden profile |
...
| #30 文档解析流水线第一阶段 | 新增 `src/documents` artifact / engine / router / quality 模块；MarkItDown 作为本地上传文件 baseline engine，依赖不可用或失败时 fallback 到 PyPdfEngine；后台 ingest 返回 202，`ingest-status` 暴露 stage/progress/selected_engine/attempted_engines/quality_summary；`TextbookExtractor` 改为消费 DocumentParseArtifact 并输出带 evidence 的 candidates；前端上传后展示解析中、轮询状态并提示 needs_ocr；新增 `document-parsing-pipeline.md` | 完整 OCR、云 Document AI、layout-aware extractor、持久化 job queue、批量人工校对体验 |
