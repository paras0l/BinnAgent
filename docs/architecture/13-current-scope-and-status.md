...
| Knowledge Base / RAG | 部分实现 | 文档解析流水线第一阶段已落地：MarkItDown baseline、pypdf fallback、免费本地 OCR、统一 DocumentParseArtifact、DocumentQuality、ParserRouter attempted/selected/fallback 记录、后台 ingest + ingest-status 轮询、metadata 状态拆分；教材抽取层改为消费 artifact 候选；另有 chunk、embedding、文本 fallback、ParserRun 审计、ParserReviewItem 队列、Parser Evidence 目标查询、Golden Dataset + Parser Evaluation MVP、8 题混合练习流、hint/retry/rubric 反馈 | layout-aware extractor、hybrid retrieval、golden query set 扩展、练习 session 总结、更多年级 golden profile |
| Prompt & Parsing Governance | 部分实现 | Prompt Registry MVP、核心 prompt 模板、写作导入 JSON-first extraction、PromptExecutor 第一阶段、`prompt_execution_records` 轻量业务记录、统一 JSON repair/schema validation、`writing_phrase.import` 迁移、Prompt Registry/PromptExecutionRecord Debug API、`scripts/evaluate_prompts.py` 和三个 prompt eval set；prompt render API 已作为 Debug API 保护，普通页面使用本地兼容生成指令 fallback；教材低置信词条支持确认、修改发布和忽略 | 迁移 vocabulary agent、exercise generation、vocabulary enrichment/detail HTML、graph feedback、essay scoring 和 dictionary lookup 到 PromptExecutor |
...
| #29 Prompt Registry + Schema-first 增强 | 新增 `PromptExecutor`、`PromptExecutionRecord`、统一 `repair.py`/`validation.py`、PromptExecutionRecord Debug API、Prompt Registry Debug API、prompt execution audit/governance 文档；`writing_phrase.import` 已接入 schema validation、repair、regex fallback 记录，fallback decision 默认为 `review_required`；新增 `scripts/evaluate_prompts.py`，读取 `PromptMetadata.eval_set` 输出 schema_pass/repair/fallback/accepted/review/confidence 指标，并在 schema pass 低于阈值时失败；`vocabulary.agent.extract`、`writing_phrase.import`、`grammar.micro_lesson.structured` eval set 和 regression tests 覆盖 schema valid/invalid、JSON repair、fallback 与非 accepted decision | 继续迁移 vocabulary agent、exercise generation、vocabulary enrichment/detail HTML、graph feedback、essay scoring、dictionary lookup 等直接 model 调用路径 |
