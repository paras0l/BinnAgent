...
| Core Runtime Upgrade | 新增 `src/runtime`、`src/evidence`、`src/mastery`、`src/recommendation`、`src/learning`、`src/verification`；Knowledge Exercise 提交写入完整 AgentEpisode trace；Daily Lesson start/answer 和 Explore skill start 接入 TaskSpec；`/runtime/episodes/:episodeId` Debug 页面已移入 Dev Console；simulation 增加 episode runtime regression | checkpoint/resume 深化、权限隔离、更多 handler 从 not_implemented 变成完整闭环 |
