...
| Evaluation / Simulation | 部分实现 | `src/simulation` 提供 deterministic learner persona、behavior policy、scenario runner、assertion engine 和结构化 simulation report；内置 scenarios 已补 `module_tags`、entrypoints、expected events/tool calls、required metrics 和 change triggers；新增 impacted simulation 推导脚本；AssertionEngine 支持 runtime event/tool/order/verification/recommendation 断言；已覆盖 smoke、vocabulary agent deposit、vocabulary practice adaptation、daily graph 基线、episode runtime knowledge practice、daily lesson checkpoint resume 和 daily_lesson_capability_recommendation；report 增加 scenario_contract、episode_count、completed_episode_count、verification_pass_count、avg_tool_latency_ms 等 runtime_metrics | 当前仍是 contract simulation 层；后续补 integration/e2e mode、baseline gates、写作好句闭环、Memory 可控性 regression、LLM-assisted learner 和 dashboard |
...
| #28 Simulation / Evaluation 增强 | 第一阶段已补 SimulationScenario contract、module_tags、entrypoints、expected events/tool calls、required metrics、change_triggers；新增 `scripts/list_impacted_simulations.py`；AssertionEngine 新增 event/tool/order/value/evidence/verification/recommendation/error 断言；runtime/daily lesson 相关 scenarios 已使用新断言 | integration/e2e mode、baseline/指标门禁、prompt/parser/rag 专项 scenario 和 dashboard 后续另做 |
