...
| Core Runtime Upgrade | 新增 `src/runtime`、`src/evidence`、`src/mastery`、`src/recommendation`、`src/learning`、`src/verification`；Knowledge Exercise 提交写入完整 AgentEpisode trace；Daily Lesson start/answer 和 Explore skill start 接入 TaskSpec；前端新增 `/runtime/episodes/:episodeId` Debug 页面；simulation 增加 episode runtime regression | checkpoint/resume 深化、权限隔离、更多 handler 从 not_implemented 变成完整闭环 |
...
- Daily lesson 已有轻量 answer_required/start/answer API，但 LangGraph checkpoint/resume 仍需深化。
