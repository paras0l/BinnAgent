...
当前阶段从“单本七年级教材”升级为“多教材版本库”。教材 source 是一等实体，前端必须允许在已上传/已发布的教材之间切换；新上传教材不能覆盖或隐藏旧教材。系统优先保证人教版七年级上/下册的结构化抽取质量，同时允许八年级及后续年级 PDF 进入同一上传、解析、审核流程。未知教材先使用通用目录/词表/RAG fallback，并把低置信内容进入人工校对队列。
...
2. **个性化学习**：系统结合教材顺序、知识依赖、用户目标和 learner memory，推荐每日课程与动态学习路径。
...
| 七年级下册 | 是，必须与上册并存，可在前端切换；词汇和语法进入同一校对队列 |
| 七年级用户持续上传的新教材/补充材料 | 是，进入同一审核与发布流程 |
| 八、九年级教材 | 是，允许上传解析；缺少专用 profile 时使用通用目录/词表/RAG fallback |
| 小学教材 | 否 |
| 高中教材 | 否 |

“允许上传解析”不等于已有完整 golden quality；七上/七下仍是质量基线，更多年级先保证不被拒绝、可生成目录/chunk、可暴露 parser evidence 和人工校对队列。

### 1.2 多教材 Source 选择规则

- `knowledge_sources` 不按年级覆盖；同一 learner 可拥有多本 private source，public source 可被复用。
- `GET /api/learners/{learner_id}/knowledge-base` 默认选择最近学习过的 source；没有学习记录时选择最近上传/发布的 source。
- 前端可传 `source_id` 明确切换教材；`node_id` 必须属于该 source。
- overview 返回 `sources[]`，包含 source id、title、grade、volume、status、unit_count、knowledge_count、requires_review 和 progress。
- 左侧教材目录标题来自当前 source，不写死“七年级上册”。
- 上传对话框不再限制七年级；支持七年级、八年级、九年级及 `grade-7/8/9` 文件名。无法识别年级时仍允许上传，但标为 `grade="unknown"` 并进入校对。
- 对没有文本层的扫描版 PDF，当前不把空抽取当成功：已知教材可使用低置信 fallback 词表/语法进入校对队列；未知教材至少生成“全册材料”节点和 RAG/chunk 处理状态。完整 OCR/layout-aware extractor 是后续质量提升项。
