...
| `exercise.unit_candidates` | `v1` | `UnitExerciseCandidatesOutput` | `default_model=ollama_utility`, `temperature=0.35`, `max_tokens=6000` | Generates a cached unit candidate pool from knowledge points and a deterministic coverage plan; schema-invalid output never reaches the exercise table. |
| `exercise.unit_review` | `v1` | `UnitExerciseReviewOutput` | `default_model=ollama_utility`, `temperature=0.0`, `max_tokens=2500` | Independently reviews every deterministic-valid candidate; only explicitly accepted, fully covered indexes may be published. |
...
- `exercise.unit_candidates` and `exercise.unit_review`: the unit exercise path uses separate generation and review schemas. Both must return `decision=accepted`; deterministic set checks run between and after the model calls. Failed generation retries once, then falls back only to curated/textbook questions, even when that means returning fewer than the requested default.
