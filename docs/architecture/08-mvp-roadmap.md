...
> 系统能围绕用户配置的学习目标和当前水平，持续记录用户错词错因，生成每日可执行任务，并通过反馈和复习帮助用户看到进步。
...
> 面向可配置学习目标的英语学习场景，设计基于 LangGraph 的长期陪伴型学习 Agent。系统读取学习目标和 CEFR 当前水平，通过 Learning Supervisor 调度 Vocabulary、Reading、Writing 等专家 Agent，将每日学习编排为诊断、训练、反馈、复习和记忆更新的状态化流程。构建学习专属 Memory，追踪错词、错因、写作错误模式、复习计划和学习节奏，并通过间隔复习和周报形成个性化提分闭环。
