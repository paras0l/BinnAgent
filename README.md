...
| Reading-led Learning Track | 核心闭环已实现并完成发布前体验加固；学习者可按水平、兴趣、薄弱点与时间预算生成短文；精读句子执行“先自主拆解、再评估教学”，知识点从 Grammar Can-Do 动态召回：有效尝试更新 production mastery 和错误模式，明确不会则展示教学复盘且不污染掌握度；沉淀复盘按“重点结论 → 逐句纠错 → Can-Do → 下一步”分层，统计图表按需展开；工作台支持按材料隔离的 7 天草稿、导航防丢失、来源溯源、幂等完成和 learner 权限校验，未评分整篇完成不会被误算为能力分或 100% 正确 |
...
| Model Provider | 部分实现；默认使用 LongCat，仍可通过环境变量或 Dev Console Debug 页面切换到 Ollama / DeepSeek；不支持原生 response format 的 provider 会收到压缩 JSON Schema，结构化修复会携带具体校验错误并再次验收；RAG embedding 暂时隔离在 Ollama 路径 |
