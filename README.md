...
| 教材 Knowledge Base / RAG / Exercises | 部分实现，作为冷启动知识来源；已支持多教材切换、文档解析流水线第一阶段（MarkItDown baseline、pypdf fallback、免费本地 OCR、DocumentParseArtifact、质量指标、后台 ingest + 状态轮询）、ParserRun 审计、ParserReviewItem 队列、Parser Evidence 查询、Dev Console Textbook Parsing Report、低置信词条人工校对入口、统一 ExerciseItem / ExerciseAttempt 语义和 AI 生成练习可编辑保存 |
