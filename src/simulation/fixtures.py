...
BUILTIN_SCENARIOS.update(
    ...
    "episode_runtime_knowledge_practice": SimulationScenario(
        ...
        module_tags=["runtime", "exercise", "mastery", "memory", "verification", "knowledge"],
        entrypoints=[
    ...
    "llm_json_missing_field_triggers_repair": SimulationScenario(
        id="llm_json_missing_field_triggers_repair",
        name="LLM JSON missing field triggers deterministic repair",
        persona_id="grade7_low_vocab",
        steps=[
            SimulationStep(
                name="create_learner",
            ...
            SimulationStep(
                name="generate_exercise_with_repair",
                action="generate_exercise_with_repair",
                assertions=[
                    {"type": "status_code", "path": "status_code", "equals": 200},
                    {"type": "not_empty", "path": "generated_exercises"},
                    {"type": "equals", "path": "fake_model.provider", "value": "deterministic_fake"},
                    {"type": "equals", "path": "fake_model.repair_used", "value": True},
                ],
            ),
        ],
        module_tags=["prompt_schema", "exercise", "integration", "model_provider"],
        entrypoints=[
            "/api/learners/{learner_id}/exercises/generate",
            "src.providers.router.ModelRouter.chat",
        ],
        expected_events=[],
        expected_tool_calls=[],
        expected_state_changes=["schema_repair_used", "generated_exercise_returned"],
        required_metrics=[
            "api_success_rate",
            "assertion_pass_rate",
