import { ArrowLeft, BookOpen, Check, Clipboard, ExternalLink, FileInput, RefreshCw } from 'lucide-react'
...
export function VocabularyDetailPage({ term, onBack, backLabel = '返回词汇练习' }: VocabularyDetailPageProps) {
  const { showToast } = useToast()
  const prompt = useMemo(() => buildVocabularyPrompt(term), [term])
  const storageKey = useMemo(() => `binnVocabularyDetail:${term.trim().toLocaleLowerCase()}`, [term])
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc] px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-7xl">
        <button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-600">
        ...
        <header className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">{term}</h1>
          <p className="mt-2 text-sm text-slate-500">聚焦一个单词或词组，理解词义层次、搭配、语境和易错用法。</p>
        </header>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
          <section className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2"><FileInput className="size-5 text-indigo-600" /><h2 className="text-lg font-black">详解阅读区</h2></div>
                <p className="mt-1 text-sm text-slate-500">粘贴 AI 返回的 HTML 后即可阅读。</p>
              </div>
              <button type="button" onClick={() => updateHtml('')} className="rounded-lg border p-2 text-slate-500 hover:bg-slate-50" title="清空内容"><RefreshCw className="size-4" /></button>
            </div>
            <div className="mt-4 min-h-[560px] overflow-hidden rounded-xl border border-slate-200 bg-white">
              {safeHtml ? (
                <iframe title={`${term} 词汇详解`} srcDoc={detailDocument(safeHtml)} sandbox="" className="h-[640px] w-full" />
              ) : (
                <div className="flex min-h-[560px] flex-col items-center justify-center px-6 text-center text-slate-400">
                  ...
                  <p className="mt-4 text-sm font-bold">复制右侧指令生成详解，再把 HTML 粘贴回来。</p>
                </div>
              )}
            </div>
          </section>
          ...
          <aside className="space-y-5">
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <p className="mt-1 text-sm text-slate-500">已将“{term}”写入专用 prompt。</p>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button type="button" onClick={() => void copyPrompt()} className="inline-flex items-center justify-center gap-2 rounded-xl border px-3 py-2.5 text-sm font-bold hover:bg-slate-50">
                  {isCopied ? <Check className="size-4 text-emerald-600" /> : <Clipboard className="size-4" />}{isCopied ? '已复制' : '复制指令'}
                </button>
                <button type="button" onClick={() => void launchTarget()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-3 py-2.5 text-sm font-bold text-white hover:bg-indigo-700">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <textarea readOnly value={prompt} className="mt-3 h-72 w-full resize-none rounded-xl border bg-slate-50 p-3 text-xs leading-relaxed outline-none" />
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              <h2 className="text-lg font-black">HTML 输入</h2>
              <textarea value={html} onChange={(event) => updateHtml(event.target.value)} placeholder="粘贴 AI 返回的 HTML 片段…" className="mt-3 h-64 w-full resize-none rounded-xl border p-3 font-mono text-xs leading-relaxed outline-none focus:border-indigo-500" />
            </section>
          </aside>
        </div>
      </div>
