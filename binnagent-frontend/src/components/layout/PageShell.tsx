...
export function PageShell({ children, className = '', contentClassName = '', variant = 'standard' }: PageShellProps) {
  ...
  return (
    <div className={`binn-min-viewport-height bg-[#f6f7f9] ${className}`}>
      <div className={`mx-auto flex w-full ${widthClass} flex-col gap-5 px-4 py-6 sm:px-6 lg:px-8 ${contentClassName}`}>
        {children}
      </div>
    </div>
  )
