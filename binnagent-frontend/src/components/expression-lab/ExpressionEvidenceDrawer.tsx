...
export function ExpressionEvidenceDrawer({ session, open, onClose }: ExpressionEvidenceDrawerProps) {
  ...
  return (
    <>
      <aside className="hidden min-h-0 overflow-y-auto border-l border-slate-200 bg-transparent py-2 pl-5 pr-2 2xl:block" aria-label="来源与证据">
        {content}
      </aside>
      {open ? (
...
function ExpressionEvidenceContent({ session }: { session: ExpressionLabSessionDetail }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border-l-2 border-indigo-200 py-1 pl-3 pr-1">
      ...
      <div className="border-l-2 border-emerald-200 bg-emerald-50/60 px-3 py-3 text-emerald-800">
